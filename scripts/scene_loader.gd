...
const SCENE_LOAD_TIME : float = 0.25
...
func fade_in_scene(scene_name : StringName, duration_out : float = SCENE_LOAD_TIME, duration_in : float = SCENE_LOAD_TIME):
	...
	fade_tween.tween_callback(ensure_game_unpaused)
	fade_tween.tween_property(color_rect, "color", normal_dim_level, duration_in)

func reload_scene(duration : float = SCENE_LOAD_TIME):
	fade_tween = create_tween()
...
func flashbang_fade_in_scene(scene_name : StringName, duration_out : float = SCENE_LOAD_TIME, duration_in : float = SCENE_LOAD_TIME):
	fade_tween = create_tween()
...
func ensure_game_unpaused() -> void:
